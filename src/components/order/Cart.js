...
Notice
    ...
    componentWillUnmount(){
        PubSub.unsubscribe(this.pubsubCmenuObj);
    }
            ...
            <List  style={{width: '100%',fontSize:'8px'}}>
                {
                    products.map((eachitem, index) => {   //console.log("eachitem"); console.log(eachitem);  console.log("index"); console.log(index);
                        if( this.state.cmenu[tabIdx].products[index].num >0 ) {
                            return (<Item
                                    key={eachitem.proId}
                                    extra={
                                        <Stepper
                                    ...
                                    multipleLine
                                >
                                    <div style={{display: 'inline-block', verticalAlign: 'middle',}}><img
                                        src={eachitem.imagin}
                                        style={{height: '80px', width: '80px', marginRight: '5px'}}/></div>
                                    <div style={{
